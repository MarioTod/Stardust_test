    ...
    try{    
          ...
          console.log("flag:"+flag.get(req.body.id_client));
           if(flag.get(req.body.id_client)==false|| flag.get(req.body.id_client)==null || req.body.on==true){
            ...
            // stringa contenente i json delle prenotazioni
            /*
             pren=JSON.stringify(result).replace('[','').replace(']','');
              tmp=JSON.stringify(result2).replace('[','').replace(']','');
          */
        ...
        try{
           ...
           // result= await pagamenti.get_single(req.body.id_client,req.body.code_prenotation);
            ...
            transporter.sendMail({
                ...
                to: email,
                subject: 'rimborso',
                text: `E' stato effettuato il rimborso totale   della prenotazione numero , a seguito della sua eliminazione `
              }, function(err, info) {
                if (err) {
                  throw err;
                } else {
